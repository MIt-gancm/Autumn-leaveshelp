---
title: linux功能
createTime: 2025/08/05 12:50:45
permalink: /article/yb56g2be/
---

# Linux环境

## 目录
1. [Minecraft 服务器管理](#minecraft-服务器管理)
   - [相关源文件](#相关源文件)
   - [系统概述](#系统概述)
   - [服务器类型](#服务器类型)
   - [服务器安装详细信息](#服务器安装详细信息)
   ...
   - [Java 管理](#java-管理)
     - [Java 安装过程](#java-安装过程)
   - [MCSManager 集成](#mcsmanager-集成)
     - [MCSManager 组件概述](#mcsmanager-组件概述)
     - [环境特定处理](#环境特定处理)
   - [服务器配置](#服务器配置)
     - [检查 Java 安装](#检查-java-安装)
     - [命令行界面](#命令行界面)
       - [菜单驱动的界面](#菜单驱动的界面)

2. [QQ 机器人管理](#qq-机器人管理)
   - [系统概述](#系统概述)
   - [NapCatQQ 管理](#napcatqq-管理)
     - [安装](#安装)
   - [WebUI 访问](#webui-访问)
   - [配置](#配置)
     - [QQbot 变量](#qqbot-变量)
...
### 相关源文件
**目的和范围**
本文档提供了有关 Autumn-leaves 系统中 Minecraft 服务器管理功能的技术详细信息。它涵盖了通过 Autumn-leaves 命令行界面安装、配置、启动和删除 Minecraft 服务器。该文档还详细介绍了用于基于 Web 的服务器管理的 MCSManager 集成。
...
### 服务器安装详细信息
#### 安装过程
该过程使用 FastMirror.net API 检索可用的服务器内核和版本。主要步骤包括：
...
7. 用户配置内存设置。
8. 系统生成启动脚本。
9. 服务器文件存储在 `$HOME/.termux/gancm/MCserver/[CoreType]/[GameVersion]/`。
...
提示用户进行内存设置（最小值和最大值），生成具有适当配置的 `start.sh` 脚本。自动接受 Minecraft EULA。对 Forge 服务器实施了特殊处理，这些服务器需要额外的安装步骤才能运行。
...
### 配置
#### QQbot 变量
该变量用于存储启动时传递给 NapCatQQ 的机器人配置参数。如果未设置此值，系统将提示输入该值并将其保存到配置文件中。
